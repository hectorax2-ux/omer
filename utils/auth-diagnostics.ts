...
export function authErrorDetails(error: unknown) {
  ...
  return {
    ...
    message: typeof value.message === "string" ? redact(value.message) : "Non-Error rejection"
  };
}

export function logAuthStage(stage: string, provider: string, result: "start" | "success" | "error", error?: unknown) {
  if (typeof __DEV__ === "undefined" || !__DEV__) return;
  const entry = { stage, provider, result, ...(error === undefined ? {} : { error: authErrorDetails(error) }) };
