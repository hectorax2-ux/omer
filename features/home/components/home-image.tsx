...
export function HomeImage({ uri, style, contentFit = "cover", transition = 180, imageFocus, showFallbackIcon = true }: {
  ...
  return (
    <View style={[styles.container, style]} accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
      {uri && !failed ? (
        <Image source={{ uri }} style={StyleSheet.absoluteFill} contentFit={contentFit} contentPosition={toExpoContentPosition(imageFocus)} cachePolicy="memory-disk" priority="normal" allowDownscaling transition={transition} onError={() => setFailed(true)} />
      ) : (
