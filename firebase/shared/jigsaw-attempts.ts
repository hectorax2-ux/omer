...
export const JIGSAW_ATTEMPTS_COLLECTION = "jigsawAttempts";

export type NormalizedJigsawAttempt = {
  id: string;
  uid: string;
  username: string;
  displayName: string;
...
export function buildJigsawAttemptId(uid: string, dayKey: string) {
...
export function normalizeJigsawAttemptFromFirebase(id: string, raw: Record<string, unknown>): NormalizedJigsawAttempt | null {
  ...
  const dayKey = stringField(raw.dayKey, todayJigsawDayKey());
